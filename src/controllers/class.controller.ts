...
const assignStudents = catchAsync(async (req: Request, res: Response) => {
    const result = await classService.assignStudentsToClass(Number(req.params.classId), req.body.studentIds);
    res.send(result);
});

const assignLecturers = catchAsync(async (req: Request, res: Response) => {
    const result = await classService.assignLecturersToClass(Number(req.params.classId), req.body.lecturerIds);
    res.send(result);
});
